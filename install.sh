...
# 1. Python package (user space), with every dependency transitively pinned
#    and hash-verified (marketplace security baseline; no floating ranges).
#    --break-system-packages for PEP 668 distros.
log "installing locked runtime deps (hash-verified) ..."
PIP_FLAGS="--user --no-cache-dir --disable-pip-version-check"
if ! python3 -m pip install $PIP_FLAGS --require-hashes -r "$REPO_DIR/requirements.lock" 2>/dev/null; then
    python3 -m pip install $PIP_FLAGS --break-system-packages --require-hashes -r "$REPO_DIR/requirements.lock"
fi
log "installing pinned build backend (hash-verified) ..."
if ! python3 -m pip install $PIP_FLAGS --no-deps --require-hashes -r "$REPO_DIR/build.lock" 2>/dev/null; then
    python3 -m pip install $PIP_FLAGS --break-system-packages --no-deps --require-hashes -r "$REPO_DIR/build.lock"
fi
log "installing python package (user, no-deps from locked tree) ..."
if ! python3 -m pip install $PIP_FLAGS --no-build-isolation --no-deps "$REPO_DIR" 2>/dev/null; then
    python3 -m pip install $PIP_FLAGS --break-system-packages --no-build-isolation --no-deps "$REPO_DIR"
fi
